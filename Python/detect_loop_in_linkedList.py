...
def detectLoop_TwoPointer(head):
    ...
    fast = head.next    # initializing fast pointer to point next node to head node of the linked list
    flag = 0        # initializing flag to 0
    while slow and fast and fast.next:  # checks if slow pointer, fast pointer and next to fast pointer are not pointing to None
        ...
        if slow == fast:
            flag = 1       # set flag to 1 if slow and fast pointers points to the same node and 
...
def detectLoop_Hashing(head):
    ...
    while temp:       # if temp node is not None, execute the following code
        ...
        if temp in hashmap:     # If we have already has this node in hashmap it means their is a cycle (Because you we encountering the node second time).
            return True
        
        hashmap.add(temp)     # If we are seeing the node for the first time, insert it in hashmap
...
def detectLoop(head):
    ...
    while (head != None):       # checking the condition, if head is None, if it is isn't None... enters the while loop.
 
        # This condition is for the case, when there is no loop
        if (head.next == None):
            return False
 
        if (head.next == temp):         # Check if next node is pointing to empty/null (since temp is empty)
            return True
